...
ruta <- "/home/juanpablo/Documentos/simulacion/instancias/simulacion/p/U/datos"
ruta2 <- "/home/juanpablo/Documentos/simulacion/instancias/simulacion/p/U/resultados"
...
##----------------------------------------------------------------------------------------##
act_obligatorias <- function(){
  seleccion <- data.frame(0) # Guarda las actividades seleccionadas
  for(o in 1:length(obligatorias)){
    act<-actividades[as.numeric(obligatorias[o]),]
    if(act[5] != 0){
      ...
      if(act[6] != 0){
        ...
        if(!any(seleccion == act[5])){
          if(!any(seleccion == act[6])){
            ...
            seleccion <- rbind(seleccion, obligatorias[o])
...
montecarlo1 <- function(i){
  ...
  if(datos[length(datos)] > 0){ #Hay actividades obligatorias
    ob <- act_obligatorias()
    for(s in 1:length(ob)){
      act <- actividades[as.numeric(ob[s]),]
      puntos[act[4]] <- puntos[act[4]] + act[1]
      copia_act[act[5],7] <- 1
      seleccion <- rbind(seleccion, as.numeric(ob[s]))
    }
  }
...
for(r in cant_repeticiones){
  for(file in files){
    ...
    print(paste("p-U",r,cont))
    print(file)
  
    #datos <-((lee(paste0(ruta,"/problem1m-1t-2s-5a-80208020U-1.pddl.txt"))))
    #datos <-((lee(paste0(ruta,"/problem1m-1t-2s-5a-0000U-1.pddl.txt"))))
    datos <-((lee(file)))
    ...
    ##------------------------------------  OBTIENE MATERIAS OBLIGATORIAS  ------------------------------##
    obligatorias <- matrix()
    if(datos[length(datos)] > 0){ #Hay actividades obligatorias
      c <- unlist(as.list(strsplit(datos[13 + as.numeric(datos[3]) + as.numeric(datos[2]) + as.numeric(datos[1]) + as.numeric(datos[5])], " ")))
      indice <- 13 + as.numeric(datos[3]) + as.numeric(datos[2]) + as.numeric(datos[1]) + as.numeric(datos[5])
      while( indice <= length(datos)){
        obligatorias <- rbind(obligatorias, c)
        indice <- indice + 1
        c <- datos[indice]
      }
      obligatorias <- matrix(obligatorias[-1])
      colnames(obligatorias)<-"obligatorias"
      }
    ##------------------------------------  ACTIVIDADES -------------------------------------##
    ...
    clusterExport(cluster, "act_obligatorias")
    clusterExport(cluster, "obligatorias")
...
tiempo_computo <- data.frame()
cluster <- makeCluster(detectCores() - 1)
tiempos_optimos <- read.table("/home/juanpablo/Documentos/simulacion/instancias/optimo/p/V.txt", sep="\n")
ruta <- "/home/juanpablo/Documentos/simulacion/instancias/simulacion/p/V/datos"
ruta2 <- "/home/juanpablo/Documentos/simulacion/instancias/simulacion/p/V/resultados"
files <- list.files(path=ruta, pattern=".txt", all.files=T, full.names=T)
##------------------------------------   LEE Y EXTRA DATOS PDDL --------------------------##
lee = function(path) {
  archivo <- data.frame()
...
ruta <- "/home/juanpablo/Documentos/simulacion/instancias/simulacion/m/U/datos"
ruta2 <- "/home/juanpablo/Documentos/simulacion/instancias/simulacion/m/U/resultados"
files <- list.files(path=ruta, pattern=".txt", all.files=T, full.names=T)
##------------------------------------   LEE Y EXTRA DATOS PDDL --------------------------##
lee = function(path) {
  archivo <- data.frame()
...
cant_repeticiones <- c(10000,20000,30000,40000, 50000, 60000)
#cant_repeticiones <- c(10000,20000,30000,40000, 100000, 500000)
for(r in cant_repeticiones){
  for(file in files){
    inicio <- Sys.time()
...
ruta <- "/home/juanpablo/Documentos/simulacion/instancias/simulacion/m/V/datos"
ruta2 <- "/home/juanpablo/Documentos/simulacion/instancias/simulacion/m/V/resultados"
files <- list.files(path=ruta, pattern=".txt", all.files=T, full.names=T)
##------------------------------------   LEE Y EXTRA DATOS PDDL --------------------------##
lee = function(path) {
  archivo <- data.frame()
...
tiempo_computo <- data.frame()
cluster <- makeCluster(detectCores() - 1)
tiempos_optimos <- read.table("/home/juanpablo/Documentos/simulacion/instancias/optimo/g/U.txt", sep="\n")
ruta <- "/home/juanpablo/Documentos/simulacion/instancias/simulacion/g/U/datos"
ruta2 <- "/home/juanpablo/Documentos/simulacion/instancias/simulacion/g/U/resultados"
files <- list.files(path=ruta, pattern=".txt", all.files=T, full.names=T)
##------------------------------------   LEE Y EXTRA DATOS PDDL --------------------------##
lee = function(path) {
  archivo <- data.frame()
...
tiempo_computo <- data.frame()
cluster <- makeCluster(detectCores() - 1)
tiempos_optimos <- read.table("/home/juanpablo/Documentos/simulacion/instancias/optimo/g/V.txt", sep="\n")
ruta <- "/home/juanpablo/Documentos/simulacion/instancias/simulacion/g/V/datos"
ruta2 <- "/home/juanpablo/Documentos/simulacion/instancias/simulacion/g/V/resultados"
files <- list.files(path=ruta, pattern=".txt", all.files=T, full.names=T)
##------------------------------------   LEE Y EXTRA DATOS PDDL --------------------------##
lee = function(path) {
  archivo <- data.frame()
...
#cant_repeticiones <- c(100,200,300,400, 500, 600)
cant_repeticiones <- c(100000)
for(r in cant_repeticiones){
  for(file in files){
    inicio <- Sys.time()
