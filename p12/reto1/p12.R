...
for(x in 1:50){
  ...
  clusterExport(cluster, "contadores")
   ...
   for(x in 1:10){
      suma <-sum(contadores[x,1:10])
      error <- (suma - contadores[x,x])/suma
      errores <- rbind(errores, error)
      }
}
write.csv(errores,"/home/pabloide/Documentos/3 Semestre/R_paralelo/p12/reto1/errores2.csv")