...
if (eucl) {
  ...
  #par(mfrow=c(1,3))
  plot(range(datos$dim),range(datos$Tiempo), type = "n", xlab = "Dimension", ylab = "Tiempo de Computo", xaxt="n", main = "Tiempo de Computo - paralelo")
  axis(1, xaxp=c(1,dimenciones,dimenciones - 1), las=2)
  lines(datos$dim, datos$Tiempo, type = 'b')
  
  png("p1er_tarea_p_origen.png")
  plot(range(datos$dim),range(datos$Origen), type = "n", xlab = "Dimension", ylab = "Regresa al Origen", xaxt="n", main = "Regresa al origen - paralelo")
  axis(1, xaxp=c(1,dimenciones,dimenciones - 1), las=2)
  lines(datos$dim, datos$Origen, type = 'b')
  
  png("p1er_tarea_p_distancia.png")
  plot(range(datos$dim),range(datos$Distacia), type = "n", xlab = "Dimension", ylab = "Distancia desde el Origen", xaxt="n", main = "Distancia - paralelo")
  axis(1, xaxp=c(1,dimenciones,dimenciones - 1), las=2)
